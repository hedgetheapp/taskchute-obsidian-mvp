...
この仕様はmain統合済み・未配布の`v0.6.68`候補を基準とする。v0.6.68はIntegratedだが、実機`NOT_VERIFIED`で、Prereleased / Verified / Releasedではない。公開済みv0.6.67以前の配布物は固定されている。過去文書と食い違う場合でも、ここではコード上の事実を優先する。意図や保証範囲をコードから確定できない箇所は「要確認」とする。
