...
| TaskMoved v4 | section移動・日付移動・同一task_id複数entry・空source section | `NOT_VERIFIED` | `NOT_VERIFIED` | `NOT_VERIFIED` | v0.6.70 multi-entry / v0.6.63 others | `NOT_VERIFIED` | multi-entry Forwardはv0.6.70 seq 2451でcurrent PASS。section移動・日付移動・empty-source等のcurrent v0.6.70回帰が未完了のため複合overallは昇格しない。 |
