...
この4件だけをv0.6.63のcurrent TaskMoved PASSとする。v0.6.63全体および他のtest caseをVerifiedへ昇格しない。
...
| TaskMoved v4 | section移動・日付移動・同一task_id複数entry・空source section | `NOT_VERIFIED` | `NOT_VERIFIED` | `NOT_VERIFIED` | v0.6.63 section / empty-source | `NOT_VERIFIED` | section移動はTMV4-CROSS-SECTION-01、empty-sourceはTMV4-EMPTY-SOURCE-01でcurrent PASS。日付移動と同一task_id複数entryが未確認のため複合行は昇格しない。 |
